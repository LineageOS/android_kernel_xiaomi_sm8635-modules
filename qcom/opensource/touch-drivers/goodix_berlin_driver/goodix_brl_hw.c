...
void brl_init_touch_filters(struct goodix_ts_core *cd) {
	memcpy(cd->touch_filters, goodix_filter_default, sizeof(cd->touch_filters));
}
...
static int brl_send_touch_filters(struct goodix_ts_core *cd) {
  const u8 *filters;
	...
	if (filters[TOUCH_FILTER_EXPERT_MODE] != GOODIX_EXPERT_LEVEL_OFF) {
		const u32 *preset =
			&cd->board_data.touch_expert[(filters[TOUCH_FILTER_EXPERT_MODE] - 1) *
	...
	cmd.len = 6;
	cmd.data[0] =
		brl_panel_orientation_bits(filters[TOUCH_FILTER_PANEL_ORIENTATION]) |
...
static int brl_set_game_mode(struct goodix_ts_core *cd, bool enabled) {
	...
	if (ret < 0) {
		ts_err("game mode: failed to send %s cmd", enabled ? "game" : "normal");
...
static int brl_set_touch_filter(struct goodix_ts_core *cd,
	...
	if (value < 0 || value > goodix_filter_max[filter]) {
		ts_err("touch filter %d: value %d outside of range 0 - %d", filter, value,
			goodix_filter_max[filter]);
		return -EINVAL;
	}
	...
	if (brl_filter_in_expert_preset(filter))
		cd->touch_filters[TOUCH_FILTER_EXPERT_MODE] = GOODIX_EXPERT_LEVEL_OFF;
