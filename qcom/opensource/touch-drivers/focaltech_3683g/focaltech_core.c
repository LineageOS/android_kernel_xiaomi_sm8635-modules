...
static const u32
    fts_touch_expert_default[FTS_EXPERT_LEVEL_NUM * FTS_EXPERT_PARAM_NUM] = {
...
static void fts_parse_dt_touch_filters(struct device_node *np,
	...
	u32 *dests[] = {pdata->touch_range, pdata->touch_def, pdata->touch_expert};
	int i, ret;
	...
	for (i = 0; i < ARRAY_SIZE(tables); i++) {
		ret = of_property_read_u32_array(np, tables[i].name, dests[i],
										tables[i].count);
		if (ret) {
		FTS_INFO("can't find %s, use defaults:%d", tables[i].name, ret);
		memcpy(dests[i], tables[i].fallback, tables[i].count * sizeof(u32));
		continue;
		}
	...
	for (i = 0; i < FTS_EXPERT_PARAM_NUM; i++) {
		...
		FTS_ERROR("touch-def-array[%d] is %d, clamping", i, pdata->touch_def[i]);
...
static u8 fts_panel_orientation_reg(u8 orientation, bool game_mode) {
	switch (orientation) {
	case 1:
		return game_mode ? FTS_EDGE_FILTER_LEFT_GAME : FTS_EDGE_FILTER_LEFT;
	case 3:
		return game_mode ? FTS_EDGE_FILTER_RIGHT_GAME : FTS_EDGE_FILTER_RIGHT;
	default:
...
static int fts_send_touch_filters(struct fts_ts_data *ts_data) {
	...
	if (ts_data->game_mode &&
		...
		const u32 *preset =
			&pdata->touch_expert[(filters[TOUCH_FILTER_EXPERT_MODE] - 1) *
								FTS_EXPERT_PARAM_NUM];
	...
	ret = fts_write_reg(
		...
		fts_panel_orientation_reg(
			ts_data->game_mode ? filters[TOUCH_FILTER_PANEL_ORIENTATION] : 0,
			ts_data->game_mode));
	if (ret < 0) {
	...
	ret =
		fts_write_reg(FTS_REG_EDGE_FILTER_LEVEL,
	...
	FTS_INFO("game mode cmd: %02X,%02X,%02X,%02X,%02X,%02X,%02X", cmd[0], cmd[1],
...
void fts_init_touch_filters(struct fts_ts_data *ts_data) {
	...
	filters[TOUCH_FILTER_TOLERANCE] = pdata->touch_def[FTS_EXPERT_TOLERANCE];
...
static int fts_set_touch_filter(struct fts_ts_data *ts_data,
	...
	if (value < 0 || value > fts_filter_max[filter]) {
		FTS_ERROR("touch filter %d: value %d outside of range 0 - %d", filter,
				value, fts_filter_max[filter]);
		return -EINVAL;
	}
	...
	if (fts_filter_in_expert_preset(filter))
		ts_data->touch_filters[TOUCH_FILTER_EXPERT_MODE] = FTS_EXPERT_LEVEL_OFF;

	ret = fts_send_touch_filters(ts_data);
	if (ret < 0) {
		FTS_ERROR("touch filter %d: failed to send cmd", filter);
		ts_data->touch_filters[filter] = previous;
		ts_data->touch_filters[TOUCH_FILTER_EXPERT_MODE] = previous_expert;
		goto exit;
	}
...
static int fts_set_cur_value(void *private, enum touch_mode mode, int value)
{
	...
	case TOUCH_MODE_PANEL_ORIENTATION:
	case TOUCH_MODE_EXPERT_MODE:
		return fts_set_touch_filter(fts_data, touch_mode_to_filter(mode),
									value);
	default:
